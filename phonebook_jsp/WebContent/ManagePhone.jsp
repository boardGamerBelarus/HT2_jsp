...
<body>
	...
	<form
		action="<%=request.getContextPath()%>/?action=edit&id=<%=person.getId()%>"
		method="post">
		...
		<input type="hidden" name="phoneId"
			value="<%=request.getAttribute("phoneId")%>" />
		<%
			}
		%>
		<table align="center" border="1" width="70%">
			...
			%>
			<tr>
				<td colspan="2"><span>Информация о телефоне владельца : <%
					out.write(person.getName() + " " + person.getSurname() + " " + person.getMiddlename());
				%>
				</span>
				</td>
			</tr>
			<tr>
				<td>Номер:</td>
				<td><input type="text" class="inputText" name="number"
					value="<%=request.getAttribute("editable_phone")%>" /></td>
			</tr>
			<tr>
