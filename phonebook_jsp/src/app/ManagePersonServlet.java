...
public class ManagePersonServlet extends HttpServlet {
	...
	private String validatePhone(String phone) {
		...
		boolean matchesArePresent = matcher.matches();

		if (!matchesArePresent) {
			error_message += "Номер должен быть строкой от 2 до 50 символов цифр, знаков '+', '-', '#'.<br />";
		}
		return error_message;
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// Если же действие указано, то...
			...
			case "add":
				...
				jsp_parameters.put("phoneAreaPresent", "false");
			...
			case "edit":
				...
				jsp_parameters.put("phoneAreaPresent", "true");
			...
			case "addPhone":
				...
				if (request.getParameter("number") != null) {
					editable_phone = request.getParameter("number");
				}
				...
				request.setAttribute("editable_phone", editable_phone);
				request.setAttribute("jsp_parameters", jsp_parameters);
			...
			case "editPhone":
				...
				editable_phone = this.phonebook.getPhone(id, phoneId);
				current_person = this.phonebook.getPerson(id);
			...
			case "deletePhone":
				...
				// Если запись удалось удалить...
				if (current_person.deletePhone(phoneId)) {
					jsp_parameters.put("current_action_result", "DELETION_SUCCESS");
					jsp_parameters.put("current_action_result_label", "Удаление выполнено успешно");
				}
				// Если запись не удалось удалить (например, такой записи
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (edit_go != null) {
			...
			Person instanceForValidation;
			
			instanceForValidation = Duplicator.duplicate(updatable_person);
			
			instanceForValidation.setName(request.getParameter("name"));
			instanceForValidation.setSurname(request.getParameter("surname"));
			instanceForValidation.setMiddlename(request.getParameter("middlename"));
	
			// Валидация ФИО.
			String error_message = this.validatePersonFMLName(instanceForValidation);
			...
			// об ошибках.
				...
				// Установка параметров JSP.
				request.setAttribute("person", instanceForValidation);
				request.setAttribute("jsp_parameters", jsp_parameters);
		...
		if (addPhone_go != null) {
			...
			String error_message = this.validatePhone(request.getParameter("number"));
	
			// Если данные верные, можно производить добавление.
			if (error_message.equals("")) {
	
				// Если запись удалось добавить...
				if (updatable_person.addPhone(request.getParameter("number"))) {
					jsp_parameters.put("current_action_result", "ADDITION_SUCCESS");
					jsp_parameters.put("current_action_result_label", "Добавление выполнено успешно");
				}
				// Если запись НЕ удалось добавить...
			...
			// об ошибках.
				...
				request.setAttribute("jsp_parameters", jsp_parameters);
				request.setAttribute("person", updatable_person);
				request.setAttribute("editable_phone", request.getParameter("number"));
				// Передача запроса в JSP.
				dispatcher_for_phoneManager.forward(request, response);
		...
		if (editPhone_go != null) {
			...
			if (error_message.equals("")) {
				...
				// Если запись удалось обновить...
				if (updatable_person.updatePhone(request.getParameter("phoneId"), request.getParameter("number"))) {
					jsp_parameters.put("current_action_result", "UPDATE_SUCCESS");
					jsp_parameters.put("current_action_result_label", "Обновление выполнено успешно");
				}
				// Если запись НЕ удалось обновить...
